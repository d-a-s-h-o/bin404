...
import (
	...
	"github.com/d-a-s-h-o/bin404/auth/apikeys"
	"github.com/d-a-s-h-o/bin404/backends"
...
func uploadPostHandler(c web.C, w http.ResponseWriter, r *http.Request) {
	if !strictReferrerCheck(r, getSiteURL(r), []string{"bin404-Delete-Key", "bin404-Expiry", "bin404-Randomize", "X-Requested-With"}) {
...
func uploadHeaderProcess(r *http.Request, upReq *UploadRequest) {
	if r.Header.Get("bin404-Randomize") == "yes" {
		upReq.randomBarename = true
	}

	upReq.deleteKey = r.Header.Get("bin404-Delete-Key")
	upReq.accessKey = r.Header.Get(accessKeyHeaderName)

	// Get seconds until expiry. Non-integer responses never expire.
	expStr := r.Header.Get("bin404-Expiry")
